...
bool Audio::readPlayListData()
{
    ...
    { // outer while
        ...
        if (strlen(pl) > 0)
            m_playlistContent.push_back(x_strdup(pl));
    ...
    { // an ID3 header could come here
        ...
        if (InBuff.writeSpace() >= availableBytes)
        {
            if (availableBytes > 1024)
                availableBytes = 1024; // 1K throttle
