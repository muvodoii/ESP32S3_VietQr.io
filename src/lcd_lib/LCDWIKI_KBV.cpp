...
{	
  ...
  pinMode(TFTLCD_D0, OUTPUT);
  pinMode(TFTLCD_D1, OUTPUT);
  pinMode(TFTLCD_D2, OUTPUT);
  pinMode(TFTLCD_D3, OUTPUT);
  pinMode(TFTLCD_D4, OUTPUT);
  pinMode(TFTLCD_D5, OUTPUT);
  pinMode(TFTLCD_D6, OUTPUT);
  pinMode(TFTLCD_D7, OUTPUT);
  digitalWrite(TFTLCD_D0, HIGH);
  digitalWrite(TFTLCD_D1, HIGH);
  digitalWrite(TFTLCD_D2, HIGH);
  digitalWrite(TFTLCD_D3, HIGH);
  digitalWrite(TFTLCD_D4, HIGH);
  digitalWrite(TFTLCD_D5, HIGH);
  digitalWrite(TFTLCD_D6, HIGH);
  digitalWrite(TFTLCD_D7, HIGH);

  	int32_t c;
	for (c = 0; c<256; c++)                  
  	{                                                
	    set_mask_buffer[c] = 0;                              
	    if ( c & 0x01 ) set_mask_buffer[c] |= (1 << TFTLCD_D0); 
	    if ( c & 0x02 ) set_mask_buffer[c] |= (1 << TFTLCD_D1); 
	    if ( c & 0x04 ) set_mask_buffer[c] |= (1 << TFTLCD_D2); 
	    if ( c & 0x08 ) set_mask_buffer[c] |= (1 << TFTLCD_D3); 
	    if ( c & 0x10 ) set_mask_buffer[c] |= (1 << TFTLCD_D4); 
	    if ( c & 0x20 ) set_mask_buffer[c] |= (1 << TFTLCD_D5); 
	    if ( c & 0x40 ) set_mask_buffer[c] |= (1 << TFTLCD_D6); 
	    if ( c & 0x80 ) set_mask_buffer[c] |= (1 << TFTLCD_D7); 
  }  
	#endif
 #endif
 	rotation = 0;
...
{
    int16_t bfa = WIDTH - top - scrollines; 
    int16_t vsp;
    int16_t sea = top;
	...
	if(lcd_driver == ID_1963)
	{
		...
		switch (rotation) 
		{
		   	case 0:
				XC=0x2B;
				YC=0x2A;
		     	val = (1<<7)|(0<<6)|(1<<5)|(0<<3); //0 degree 
		     	break;
		   	case 1:
				XC=0x2A;
				YC=0x2B;				
		     	val = (0<<7)|(0<<6)|(0<<5)|(0<<3); //90 degree 
		     	break;
		 	case 2:
				XC=0x2B;
				YC=0x2A;
		     	val = (0<<7)|(1<<6)|(1<<5)|(0<<3); //180 degree 
		    	break;
		   	case 3:
				XC=0x2A;
				YC=0x2B;
		     	val = (1<<7)|(1<<6)|(0<<5)|(0<<3); //270 degree
		     	break;
			default:
				break;
		 }
...
void LCDWIKI_KBV::Set_1963_PWM(uint8_t value)
{
	CS_ACTIVE;
	uint8_t pwm=value*2.55;
	writeCmd16(0xBE);	//配置PWM输出
	writeData8(0x05);	//1设置PWM频率
	writeData8(pwm);//2设置PWM占空比
	writeData8(0x01);	//3设置C
	writeData8(0xFF);	//4设置D
	writeData8(0x00);	//5设置E
	writeData8(0x00);	//6设置F
	CS_IDLE;
}
...
*/
		...
		case 0x1963:
			...
			//WIDTH = 128,HEIGHT = 160;
			//width = WIDTH, height = HEIGHT;
			XC=0x2B,YC=0x2A,CC=ILI9341_MEMORYWRITE,RC=HX8357_RAMRD,SC1=0x33,SC2=0x37,MD=ILI9341_MADCTL,VL=0,R24BIT=0;
			static const uint8_t SSD1963_regValues[] PROGMEM = 
			{
				0xE2, 3, 0x1E, 0x02, 0x54, //0x1D, 0x02, 0x04,  
				TFTLCD_DELAY8, 10,
            	0xE0, 1, 0x01,
				TFTLCD_DELAY8, 10,
            	0xE0, 1, 0x03,
            	...
            	0x01, 0,
            	TFTLCD_DELAY8, 100,
            	0xE6, 3, 0x03, 0xFF, 0xFF,   //0x03      
            	0xB0, 7, 0x20, 0x00, 0x03, 0x1F, 0x01, 0xDF, 0x00,
            	0xB4, 8, 0x03, 0xA0, 0x00, 0x2E, 0x30, 0x00, 0x0F, 0x00,//0x04, 0x1F, 0x00, 0x2E, 0x00, 0x00, 0x00, 0x00, // 
            	0xB6, 7, 0x02, 0x0D, 0x00, 0x10, 0x10, 0x00, 0x08, //0x02, 0x0C, 0x00, 0x17, 0x15, 0x00, 0x00,//     
				//0xF0, 1, 0x00,
				//0x29, 0,
				//0xD0, 1, 0x00,
				//0xBE, 6, 0x05, 0xFE, 0x01, 0x00, 0x00, 0x00,
				//0xB8, 2, 0x03, 0x01,
				//0xBA, 1, 0x01
				0xBA, 1, 0x0F, 
            	0xB8, 2, 0x07, 0x01, 
			...
			init_table8(SSD1963_regValues, sizeof(SSD1963_regValues));
			break;
		default:
