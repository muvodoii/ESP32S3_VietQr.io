...
class LCDWIKI_TOUCH
{
  public:
	...
	uint16_t x0,y0,x,y,wid,heig,*cx,*cy;
	uint8_t touch_statue,touch_rotation,lcd_rotation,*csta,cid;	
	//GT911 cgt911;
  private:
 	#ifdef __AVR__
	...
	#else
		volatile uint32_t *tcsPort, *tclkPort, *tdoutPort, *tdinPort, *tirqPort;
		uint32_t  tcsPinSet, tclkPinSet,	tdoutPinSet, tdinPinSet, tirqPinSet;
	#endif
};
