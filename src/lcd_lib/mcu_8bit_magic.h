...
#define USE_ADAFRUIT_SHIELD_PIN
...
#define WR_PIN    4  // Write strobe control pin - must use a pin in the range 0-31
...
// Mask for the 8 data bits to set pin directions
 ...
 // Write 8 bits to TFT
#define write8(C)	{GPIO.out_w1tc = clr_mask; GPIO.out_w1ts = set_mask((uint8_t)C); WR_STROBE;}
#define read8(dst)
...
// Stuff common to all Arduino AVR board types:
 ...
 #define RD_ACTIVE  GPIO.out_w1tc = (1 << RD_PIN)
 #define RD_IDLE    GPIO.out_w1ts = (1 << RD_PIN)
 #define WR_ACTIVE  GPIO.out_w1tc = (1 << WR_PIN)
 #define WR_IDLE    GPIO.out_w1ts = (1 << WR_PIN)
 #define CD_COMMAND GPIO.out_w1tc = (1 << CD_PIN)
 #define CD_DATA    GPIO.out_w1ts = (1 << CD_PIN)
#if CS_PIN >= 32
	#define CS_ACTIVE GPIO.out1_w1tc.val = (1 << (CS_PIN - 32))
	#define CS_IDLE   GPIO.out1_w1ts.val = (1 << (CS_PIN - 32))
#elif CS_PIN >= 0
	#define CS_ACTIVE GPIO.out_w1tc = (1 << CS_PIN)
	#define CS_IDLE   GPIO.out_w1ts = (1 << CS_PIN)
#endif
...
#if defined(ARDUINO_ARCH_ESP32)
#define writeData18(x)  {CD_DATA; write8(uint8_t((x>>8)&0xF8));write8((uint8_t)((x>>3)&0xFC));write8(uint8_t(x<<3));}
#else
